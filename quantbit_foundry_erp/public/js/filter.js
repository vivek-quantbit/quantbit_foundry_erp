...
frappe.custom.set_filters_for_doctype = async function(doctype_name, frm) {
    const doctype = await frappe.db.get_doc("Foundry Filter Setting DocType", doctype_name)
    details = doctype.foundry_filter_setting_doctype_details
    details.forEach(async function(item) {
        ...
        let filter = await get_filter_list(frm, doctype_name, item.docchild_name , item.docfield_name)
        // console.log(filter)
        // console.log(item.doclink_name)
        if (child_field) {
...
async function get_filter_list(frm, filters_parent, docchild_name , docfield_name ) {
    let filter_list = []
    const val = await frappe.db.get_doc("Foundry Filter Setting DocType", filters_parent)
    data = val.foundry_filter_setting_doctype_details
    data.forEach(function(item) {
        if (item.docchild_name === docchild_name && item.docfield_name === docfield_name) {
            let filter_arg
            if (item.filterfield_field) {
                filter_arg = frm.doc[item.filterfield_field];
            } else {
